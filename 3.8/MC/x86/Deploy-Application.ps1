...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		if (Test-Path -Path $c2rConfigRegPath -ErrorAction SilentlyContinue) {
			...
			if ($c2rChannel.CDNBaseUrl -notmatch 'http://officecdn.microsoft.com/pr/492350f6-3a01-4f97-b9c0-c7c6ddf67d60') {
				Write-Log -Message "Office 365 ProPlus will be migrate to Monthly Channel." -Severity 1 -Source $deployAppScriptFriendlyName
				$migrateChannel = $true
			}
		...
		foreach ($officeProduct in $officeProducts) {
			if ($officeProduct.DisplayName -match 'office professional plus 2007') {
				Write-Log -Message "$($officeProduct.DisplayName) is installed. Will be removed to install Office 365 ProPlus (Monthly Channel)." -Severity 1 -Source $deployAppScriptFriendlyName
				$office2k7 = $true
			}
...
"@
		...
		Write-Log -Message "The final Office 365 ProPlus 32-bit (Monthly Channel) confirugarion will be written at `"$configXmlFilePath`"" -Severity 1 -Source $deployAppScriptFriendlyName
